...
# TODO: Change this to your local metl repo location
METL_REPO_PATH = "./metl"

sys.path.append(join(METL_REPO_PATH, "code"))
import datamodules
import models
import tasks
from training_utils import save_metrics_ptl
...
def main(args):
    ...
    if args.dataset_type == "dms":
        ...
        dm = datamodules.RosettaDataModule(**vars(args), target_group="standard")
    else:
    ...
    test_metrics = trainer.test(model=lm, datamodule=dm)
    save_metrics_ptl(f"{args.pretrained_ckpt_path}", "", test_metrics, output_dir)
